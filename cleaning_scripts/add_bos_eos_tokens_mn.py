...
def add_eos_tokens(text):
    ...
    Supports both Latin (A-Z) and Cyrillic (А-Я, Ё) capital letters for Mongolian."""
    ...
    # Matches both Latin [A-Z] and Cyrillic [А-ЯЁ] capitals
    text = re.sub(r'(?<!\.)\.(?!\.)\s*\)(\s*)(?=[A-ZА-ЯЁ])', r'.) <EOS> <BOS> <BOS> <BOS> ', text)
    ...
    # Matches both Latin [A-Z] and Cyrillic [А-ЯЁ] capitals
    text = re.sub(r'(?<!\.)\.(?!\.)\s*(?=[A-ZА-ЯЁ])', r'. <EOS> <BOS> <BOS> <BOS> ', text)
...
if __name__ == "__main__":
    process_file_with_eos('cc100_mn_cleaned.jsonl', 'cc100_mn_cleaned2.jsonl')